...
const TrackCard = ({
  ...
  return (
    <div className="bg-white rounded-[1.75rem] overflow-hidden border-2 border-white hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 flex flex-col h-[240px] w-full max-w-[340px] mx-auto">
      <div className="flex items-start justify-between p-5 pb-3">
        <h3 className="text-3xl font-bold text-black leading-tight">{title}</h3>
        <div className="text-black flex-shrink-0 ml-2">
          <StackIcon />
        </div>
      </div>
      <div className="bg-black mx-[0.15rem] mb-[0.15rem] rounded-b-[1.5rem] p-5 flex-1 flex items-center justify-center">
...
export default function TracksComponent() {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-8">
      <div className="bg-black rounded-[3rem] w-full max-w-[1600px] px-24 py-16">
        <h1 className="text-[5.5rem] font-black text-white mb-12 tracking-tight leading-none">
          TRACKS
        </h1>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10 px-8">
          {tracks.slice(0, 3).map((track, index) => (
            <TrackCard
              key={index}
              title={track.title}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-8 max-w-[900px] mx-auto">
          {tracks.slice(3).map((track, index) => (
            <TrackCard
              key={index + 3}
              title={track.title}
